...
<div id="main-content" class="ml-64 p-4">
    ...
    <h4 class="text-lg font-semibold mb-2">Edit DPT</h4>
    <form action="edit/update" method="post">
        <?php foreach ($rows as $row): ?>
            <input type="hidden" name="id" value="<?php echo $row['id_pemilih']; ?>">
            <label for="nim">
                Nomor Induk Mahasiswa
                <input type="text" placeholder="NIM" id="nim" name="nim" value="<?php echo $row['nim']?>">
            </label>
            <label for="nama">
                Nama
                <input type="text" placeholder="Nama" id="nama" name="nama" value="<?php echo $row['nama']?>">
            </label>
        <?php endforeach; ?>
